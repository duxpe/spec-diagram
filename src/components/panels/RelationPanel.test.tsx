...
describe('RelationPanel', () => {
  ...
  it('shows only N3 relation whitelist when level is N3', () => {
    render(<RelationPanel level="N3" nodes={nodes} onCreateRelation={vi.fn()} />)

    const relationTypeSelect = screen.getByLabelText('Type')
    expect(relationTypeSelect).toHaveTextContent('uses')
    expect(relationTypeSelect).toHaveTextContent('depends on')
    expect(relationTypeSelect).toHaveTextContent('exposes')
    expect(relationTypeSelect).not.toHaveTextContent('calls')
    expect(relationTypeSelect).not.toHaveTextContent('implements')
  })
})
